...
def download_drm_content(mpd_url):
	...
	os.system(f'yt-dlp --no-warnings --allow-unplayable-formats --no-check-certificate -F "%s"'%mpd_url)
	divider()
	...
	os.system(f'yt-dlp -o "{TEMPORARY_PATH}/encrypted_audio.m4a" --no-warnings --external-downloader aria2c --allow-unplayable-formats --no-check-certificate -f {AUDIO_ID} "{mpd_url}"')
